				...
				case 'moonlight':
					{
						camZoom = 0.8;
						curStage = 'moonlight';
						var bg:FlxSprite = new FlxSprite(-330, -200).loadGraphic(Paths.image('saber_bg'));
						bg.antialiasing = FlxG.save.data.antialiasing;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						swagBacks['bg'] = bg;
						...
						//var CrystalSPEAKER:FlxSprite = new FlxSprite(0, 200).loadGraphic(Paths.image('Crystals/CRYSTAL BEHIND SPEAKER'));
						//stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
						//stageFront.updateHitbox();
						//CrystalSPEAKER.antialiasing = FlxG.save.data.antialiasing;
						//CrystalSPEAKER.scrollFactor.set(0.9, 0.9);
						//CrystalSPEAKER.active = false;
						//swagBacks['CrystalSPEAKER'] = CrystalSPEAKER;
                        //toAdd.push(CrystalSPEAKER);
