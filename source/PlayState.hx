...
{
	static inline final ALLURE = 'allure';
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson(ALLURE, ALLURE);
	...
	function startSong():Void
	{
		...
		switch (curSong)
		{
			case 'allure' | 'Philly Nice' | 'Blammed' | 'Cocoa' | 'Eggnog':
	...
	floatshit += 0.03;
		...
		if (controls.PAUSE && startedCountdown && canPause && !cannotDie)
		{
			...
			// hehe i set it to 5% for the trolling
	...
	override function beatHit()
		{
			...
			if (curSong.toLowerCase() == 'allure')
				{
					if (curBeat > 389 && curBeat < 421)
					minusHealth = true;

					if (curBeat > 421)
					minusHealth = false;

					if (curBeat > 453 && curBeat < 484)
					minusHealth = true;
